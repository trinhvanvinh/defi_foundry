...
library Pool {
    ...
    function setHookFees(
        State storage self,
        uint8 newHookSwapFee,
        uint8 newHookWithdrawFee
    ) internal {
        if (self.slot0.sqrtPriceX96 == 0) revert PoolNotInitialized();
        ...
        )
    {
        ...
        SwapCache memory cache = SwapCache({
            ...
            protocolFee: params.zeroForOne
                ? (slot0Start.protocolSwapFee % 16)
                : (slot0Start.protocolSwapFee >> 4),
            hookFee: params.zeroForOne
                ? (slot0Start.hookSwapFee % 16)
                : (slot0Start.hookSwapFee >> 4)
        });
